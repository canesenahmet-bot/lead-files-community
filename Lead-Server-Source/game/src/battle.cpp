...
int CalcAttBonus(LPCHARACTER pkAttacker, LPCHARACTER pkVictim, int iAtk)
{
	...
	//2013/01/17
	// Monster attribute attack damage 30% Resistance is applied only to values ​​corresponding to .
	if (pkAttacker->IsNPC() && pkVictim->IsPC())
	{
...
DWORD GET_ATTACK_SPEED(LPCHARACTER ch)
{
	...
	DWORD default_bonus = SPEEDHACK_LIMIT_BONUS * 3;    // Yoo Duri’s attack speed ( basic 80) ( General users speed hack To prevent getting caught in *3 addition . 2013.09.11 CYH)
